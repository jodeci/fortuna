...
class SupplementHealthInsuranceTest < ActiveSupport::TestCase
  def test_supplement_premium_for_insured_salary
    ...
    payroll = create(:payroll, year: 2018, month: 3, employee: employee, salary: salary)
    assert_equal SupplementHealthInsuranceService.call(payroll), 0
  end
  ...
  def test_supplement_premium_for_uninsured_salary
    ...
    payroll = create(:payroll, year: 2018, month: 3, employee: employee, salary: salary)
    assert_equal SupplementHealthInsuranceService.call(payroll), 688
  end
