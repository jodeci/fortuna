...
module Calculatable
  ...
  # 已代扣所得稅的淨所得（約聘）
  def regular_paid_amount
    contractor? ? income_after_withholdings : income_before_withholdings
  end

  # 薪資所得包括代扣的勞健費在內
  def taxable_income
    regular_paid_amount + labor_insurance + health_insurance - subsidy_income
  end
  ...
  def contractor?
    payroll.salary.role == "contractor"
  end
